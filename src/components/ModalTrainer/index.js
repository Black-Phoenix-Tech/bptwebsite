...
const ModalTrainer = ({ item, visible, onClose }) => {
    ...
    return (
        <Modal containerClassName={styles.container} visible={visible} onClose={onClose}>
            ...
            <Subscription className={styles.subscription} placeholder="Find a class" />
                ...
                <Slider className="trainer-slider" {...settings}>
                    {item.courses.map((x, index) => (
                        <div className={styles.slide} key={index} onClick={onClose}>
                            <Link className={cn("trainer-item", styles.item)} href={x.url}>
                                <div className={styles.preview}>
                                    <img srcSet={x.image} src={`${x.image2x} 2x`} alt="Card pic" />
                                    <div className={cn({ "status-white": x.category === "white" }, styles.category)}>
                                        {x.categoryText}
                                    </div>
                                </div>
            ...
            <div className={styles.btns}>
                <button className={cn("button-small", styles.button)}>Browse all class</button>
            </div>
        </Modal>
    );
};
